...
const DEMO_STEPS = [
  {
    ...
    desc: 'Prefeitura de São Exemplo, SP | BC-2026-0001 | R$ 120.000,00 | Empenho: NE 2026/042 aprovado pelo Jurídico.',
    icon: FileText,
  },
  {
    id: 2,
    title: 'Criação do Pedido (PED-2026-0001)',
    desc: 'Distribuição para 3 Escolas: João Paulo I, EMEI Girassol, Pedro Álvares. 1.500 Itens gerados.',
    icon: Package,
  },
  {
    id: 3,
    title: 'Separação e Romaneio',
    desc: 'OS de separação concluída e motorista João Marcos atribuído ao romaneio (LogExpress).',
    icon: Truck,
  },
  {
    id: 4,
    title: 'Entrega Realizada',
    desc: 'Confirmação via app do motorista e upload do canhoto assinado na escola João Paulo I.',
    icon: CheckCircle2,
  },
  {
    id: 5,
  ...
  },
  {
    ...
    desc: 'Cliente reporta via Tracker: "Faltam 10 livros na João Paulo I". Alerta acionado no Dashboard.',
    icon: AlertCircle,
  },
  {
    id: 7,
    title: 'Resolução de Divergência',
    desc: 'Dossiê analisado, reenvio autorizado (Nova OS) e cliente notificado com sucesso. Chamado Finalizado.',
    icon: CheckCircle2,
  },
]
...
export default function DemoFlowPage() {
  ...
  const advanceStep = () => {
    if (currentStep < DEMO_STEPS.length) {
      ...
      toast({
        ...
        description: `Passo ${currentStep + 1} executado e KPIs atualizados.`,
      })
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in-up max-w-6xl mx-auto pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-brand-primary p-8 rounded-2xl text-white shadow-soft bg-gradient-to-r from-[#003B73] to-[#1565A8]">
        <div>
          <h1 className="text-3xl font-title font-bold tracking-tight mb-2">
            Ambiente de Demonstração End-to-End
          </h1>
          <p className="text-blue-100/90 text-sm max-w-2xl leading-relaxed font-medium">
            Simule o ciclo completo de um pedido com os dados controlados da Prefeitura de São
            Exemplo. Valide a integração entre módulos desde a formalização até a resolução de
            problemas.
          </p>
        </div>
        <Button
        ...
        >
          <RotateCcw className="w-4 h-4 mr-2" /> Resetar Demo Data
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-4">
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isActive
                    ? 'bg-white shadow-soft border-2 border-brand-orange scale-[1.02]'
                    : 'hover:bg-slate-50',
              ...
              >
                ...
                <div className="pt-1">
                  <p
                    className={cn(
                      'font-bold text-sm leading-tight mb-1 font-title',
        ...
        <div className="md:col-span-2">
          <Card className="h-full border-0 shadow-soft overflow-hidden rounded-2xl bg-white flex flex-col">
            <CardHeader className="bg-slate-50/80 border-b border-slate-100 pb-5">
              <div className="flex justify-between items-center">
                <div>
                  <CardTitle className="text-brand-primary font-title">
              ...
              <div className="flex-1 p-8 flex flex-col items-center justify-center min-h-[450px] text-center bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-slate-50/50">
                {currentStep === 0 && (
                  <div className="animate-fade-in-up space-y-4 max-w-sm">
                    ...
                    <h3 className="text-2xl font-title font-bold text-slate-800">
                      Pronto para Iniciar?
                    </h3>
                    <p className="text-slate-500 text-sm">
                      Clique no botão abaixo para disparar o pipeline de simulação da Prefeitura de
                      São Exemplo.
                    </p>
                  </div>
                )}
                ...
                {currentStep > 0 && currentStep < 7 && (
                  <div className="animate-fade-in-up w-full max-w-lg">
                    <div className="p-8 bg-white rounded-2xl shadow-soft border border-slate-100 text-left relative overflow-hidden">
                      ...
                      <h4 className="font-title font-bold text-xl mb-3 text-slate-800">
                      ...
                      {currentStep === 4 && (
                        <div className="w-full h-40 bg-slate-50 rounded-xl border-dashed border-2 border-slate-300 flex flex-col items-center justify-center text-slate-400 text-xs shadow-inner">
                          <CheckCircle2 className="w-8 h-8 mb-3 text-status-completed" />
                      ...
                      {currentStep === 6 && (
                        <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-red-800 text-sm font-medium flex items-start gap-3">
                          <AlertCircle className="w-6 h-6 shrink-0 text-status-error mt-0.5" />
                          <p>
                ...
                {currentStep === 7 && (
                  <div className="animate-fade-in-up space-y-4 max-w-md">
                    <div className="w-24 h-24 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6 text-status-completed shadow-inner">
                    ...
                    <p className="text-slate-600 text-sm leading-relaxed font-medium">
                      O pedido foi entregue, a divergência foi tratada e o cliente finalizado. A
                      demonstração End-to-End foi validada com sucesso e refletida nos Dashboards.
                    </p>
                  </div>
                )}
              </div>
              <div className="p-6 border-t border-slate-100 bg-white flex justify-center">
                {currentStep < 7 ? (
                  <Button
                    size="lg"
                    onClick={advanceStep}
                    className="bg-brand-orange hover:bg-brand-orange/90 text-white font-bold text-lg px-10 h-14 rounded-full shadow-soft hover:shadow-lg transition-all hover:-translate-y-1 w-full max-w-sm"
                  >
